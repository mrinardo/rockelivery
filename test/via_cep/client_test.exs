defmodule Rockelivery.ViaCep.ClientTest do
  # async: para rodar teste em paralelo
  ...
  describe "get_cep_info/1" do
    ...
    test "when cep is valid, returns cep's info", %{bypass: bypass} do
      cep = "01001000"
    ...
    test "when there is a generic sever error, returns an error", %{bypass: bypass} do
      cep = "00000000"

      url = endpoint_url(bypass.port)

      # Simula servidor inacessível
      Bypass.down(bypass)
